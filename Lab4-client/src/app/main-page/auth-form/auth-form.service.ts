...
export class AuthFormService{
  ...
  tryToLogin(user: User): Observable<any>{
    ...
    // @ts-ignore
    return this.http.get<any>('http://localhost:4200/api/authorization/signin', user, {headers: myHeaders, responseType: 'text'});
  }

  superAgentLogin(user: User): any{
    request
      .get('http://localhost:4200/api/authorization/signin')
